...
class JobEnv:
    def __init__(self, case_name, path='../all_data_set/', no_op=False):
        ...
        if no_op:
            ...
            # self.action_num = int(len(self.pdr_label)/2)+1
        else:
        ...
        # state features are solution presented by a mxj matrix
    ...
    def reset(self):
        ...
        self.done = False
        self.no_op_cnt = 0
        # the state is presented by operation order of solution
        self.state = np.ones([self.job_num, self.machine_num], dtype=float)
        self.solution_op_cnt = 0
    ...
    def _get_state(self):
        return np.array(self.state).reshape((1, self.job_num, self.machine_num))
    ...
    def step(self, action):
        ...
        # if sum(self.current_op_of_job) == 10 or sum(self.current_op_of_job) == 20:
        #     print("hello")
    ...
    def time_advance(self):
        ...
        # average_machine = int(2*self.total_process_time/self.make_span)-1
